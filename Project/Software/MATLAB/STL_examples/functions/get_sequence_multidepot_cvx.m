function sequence = get_sequence_multidepot_cvx(initial_position, goal, n_drones, n_targets, delta_max)
% The "get_sequence_multidepot_cvx" MATLAB function provides the inspection
% sequence for all the drones and target regions provided as input. It is 
% equivalent to the "get_sequence_multidepot_cvx" MATLAB function, but now 
% the problem is set up using CVX as LP solver (and not the MATLAB tool). 
% This makes the execution independent by the release of MATLAB.
%
% Inputs:
...
% More information on how to setup the problem are available at this link
...
% http://cvxr.com/news/2012/08/midcp/

%%%         CVX Problem setup 
%%%
%%% Problem setup (it is written in LaTeX)
%%% min_x f^\top x
%%% s.t.
%%% A x \leq b
...
% Start CVX code in quiet mode (no verbose)
cvx_begin quiet

%%%% Create the linear constraints that each stop has two associated trips, because there 
%%%% must be a trip to each stop and a trip departing each stop.
%%%% All decision variables are binary. Now, set the intcon argument to the number of decision 
%%%% variables, put a lower bound of 0 on each, and an upper bound of 1.
%%%%
%%%% ATTENTION: In CVX a binary vector is defined using "integer" while a binary 
%%%% matrix is defined using "binary"

for i = 1: n_drones
    eval([ 'variable x' num2str(i) '_1(n_targets,1) integer;' ]);
    eval([ 'variable x' num2str(i) '_2(n_edges_drone-n_targets,1) binary;' ]);
end

variable x2(n_combos_drones,1);

variable x3(n_targets*n_drones,1) binary;

string = '[';

for i = 1:n_drones
   string = [string 'x' num2str(i) '_1;' 'x' num2str(i) '_2;' ]; 
end
...
objective_fcn_cvx = [];
for kk = 1 : (n_edges + n_combos_drones + n_targets*n_drones)
    objective_fcn_cvx = [objective_fcn_cvx; objective_fcn(kk) * x(kk)];
end

minimise sum(objective_fcn_cvx) 
...
%%%%
% Equality constraints
for i = 1 : n_targets + n_targets*n_drones + n_drones
    Aeq(i,:) * x == beq(i); 
end
...
%%%%
% Inequality constraints -- CVX
for i = 1 : 2*n_combos_drones
    A(i,:) * x <= b(i); 
end
...
%%%%                                BOUNDINGS 
%%%%
% Bounding the minimization variable
for i = 1 : (n_edges+n_combos_drones+n_targets*n_drones)
    x(i) <= ub(i);
    x(i) >= lb(i);
end
...
x([1:n_edges, n_edges+n_combos_drones+1 : n_edges+n_combos_drones+n_targets*n_drones]) = ...
    round(x([1:n_edges, n_edges+n_combos_drones+1 : n_edges+n_combos_drones+n_targets*n_drones]));
