function rho_unsafe = robustness_unsafe_Belta(pos_x, pos_y, pos_z, drone_ref, ...
    optimizationParameters)
% The "robustness_unsafe_Belta" MATLAB function produces as output the set of 
% waypoints not safe. Always not unsafe in x y z. The function uses the
...
for j = 1 : optimizationParameters.obstacle_elements
    ...
    for i = 1 : numel(rho_lb_pos_x)
        ...
        temp(i, j) = SmoothMin_Belta(temp_vec,C); % Unsafe specification, in the obstacle
        
    end
    
    temp_unsafe(j) = SmoothMin_Belta(-temp(:,j),C); % Implements NOT (NOT in the obstacles)

end

rho_unsafe = SmoothMin_Belta(temp_unsafe,C); % Implements ALWAYS
